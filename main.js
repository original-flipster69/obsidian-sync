...
function resolveCredentials(app, settings) {
  const accessKey = settings.accessKeySecret ? app.secretStorage.getSecret(settings.accessKeySecret) || "" : "";
  const secretKey = settings.secretKeySecret ? app.secretStorage.getSecret(settings.secretKeySecret) || "" : "";
  return { accessKey, secretKey };
}
var OvhSyncSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Region").setDesc("OVH Cloud region for your Object Storage").addDropdown((dropdown) => {
      for (const r2 of OVH_REGIONS) {
        dropdown.addOption(r2.value, r2.label);
      }
    ...
    new import_obsidian.Setting(containerEl).setName("Custom endpoint").setDesc("Override the S3 endpoint URL (leave empty to use the region default)").addText(
      (text) => text.setPlaceholder("https://s3.gra.io.cloud.ovh.us").setValue(this.plugin.settings.customEndpoint).onChange(async (value) => {
        this.plugin.settings.customEndpoint = value;
        await this.plugin.saveSettings();
      })
    );
    const accessKeySetting = new import_obsidian.Setting(containerEl).setName("Access key").setDesc("S3 access key (stored in Obsidian secure storage)").addComponent(
      (el) => new import_obsidian.SecretComponent(this.app, el).setValue(this.plugin.settings.accessKeySecret).onChange(async (secretName) => {
        this.plugin.settings.accessKeySecret = secretName;
        await this.plugin.saveSettings();
      })
    );
    const secretKeySetting = new import_obsidian.Setting(containerEl).setName("Secret key").setDesc("S3 secret key (stored in Obsidian secure storage)").addComponent(
      (el) => new import_obsidian.SecretComponent(this.app, el).setValue(this.plugin.settings.secretKeySecret).onChange(async (secretName) => {
        this.plugin.settings.secretKeySecret = secretName;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Bucket").setDesc("S3 bucket name").addText(
      (text) => text.setPlaceholder("my-obsidian-vault").setValue(this.plugin.settings.bucket).onChange(async (value) => {
        this.plugin.settings.bucket = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Prefix").setDesc("Optional path prefix within the bucket (e.g. 'vault1/')").addText(
      (text) => text.setPlaceholder("vault1/").setValue(this.plugin.settings.prefix).onChange(async (value) => {
        this.plugin.settings.prefix = value;
        await this.plugin.saveSettings();
      })
    );
    const testSetting = new import_obsidian.Setting(containerEl).setName("Test connection").setDesc("Verify that the S3 credentials and bucket are valid").addButton(
      (button) => button.setButtonText("Test").onClick(async () => {
        ...
        try {
          ...
          testSetting.setDesc(`Error: ${e2 instanceof Error ? e2.message : String(e2)}`);
        }
        setTimeout(() => {
    ...
    new import_obsidian.Setting(containerEl).setName("Auto-sync interval").setDesc("Full sync interval in minutes (0 = disabled, on-change sync still works)").addText(
    ...
    new import_obsidian.Setting(containerEl).setName("Debounce delay (ms)").setDesc("Wait this long after a file change before syncing").addText(
      (text) => text.setPlaceholder("3000").setValue(String(this.plugin.settings.debounceDelay)).onChange(async (value) => {
        const num = parseInt(value, 10);
        this.plugin.settings.debounceDelay = isNaN(num) ? 3e3 : Math.max(500, num);
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Exclude patterns").setDesc("File/folder paths to exclude from sync (one per line, prefix match)").addTextArea(
      (text) => text.setPlaceholder(".obsidian/workspace.json").setValue(this.plugin.settings.excludePatterns).onChange(async (value) => {
...
var ObsidianHttpHandler = class {
  async handle(request, _options) {
...
var S3Client2 = class {
  ...
  async listObjects(continuationToken) {
    const prefix = this.config.prefix ? this.config.prefix.replace(/\/+$/, "") + "/" : void 0;
    ...
    const objects = (result.Contents || []).filter((obj) => obj.Key).map((obj) => {
      const key = this.unprefixedKey(obj.Key);
      return { key, lastModified: obj.LastModified || /* @__PURE__ */ new Date(), size: obj.Size || 0, etag: (obj.ETag || "").replace(/"/g, "") };
  ...
  async testConnection() {
    try {
      ...
      console.error("OVH Sync: test connection failed:", e2);
...
function computeSyncActions(localFiles, remoteFiles, tracked) {
  const actions = [];
  const localMap = new Map(localFiles.map((f2) => [f2.path, f2]));
  const remoteMap = new Map(remoteFiles.map((f2) => [f2.path, f2]));
  ...
  for (const path of allPaths) {
    ...
    if (local && remote && prev) {
      const localChanged = local.mtime > prev.mtime + MTIME_TOLERANCE_MS || local.size !== prev.size;
...
// src/sync/engine.ts
var MAX_DELETE_FRACTION = 0.5;
var MIN_DELETE_THRESHOLD = 5;
var SyncEngine = class {
  constructor(vault, s3, excludePatterns, getMetadata, saveMetadata) {
    this.vault = vault;
    this.s3 = s3;
    ...
    try {
      ...
      if (isFirstSync) {
        actions = actions.filter((a2) => {
          if (a2.type === "deleteRemote" || a2.type === "deleteLocal") {
            result.skippedDeletes++;
            return false;
          }
          return true;
        });
        actions = actions.map((a2) => {
          if (a2.type === "conflict") {
            return { ...a2, resolution: "download" };
          }
          return a2;
        });
      }
      if (!isFirstSync) {
        ...
        const deleteRemoteCount = actions.filter((a2) => a2.type === "deleteRemote").length;
        const deleteLocalCount = actions.filter((a2) => a2.type === "deleteLocal").length;
        const tooManyRemoteDeletes = trackedCount > MIN_DELETE_THRESHOLD && deleteRemoteCount > trackedCount * MAX_DELETE_FRACTION;
        const tooManyLocalDeletes = trackedCount > MIN_DELETE_THRESHOLD && deleteLocalCount > trackedCount * MAX_DELETE_FRACTION;
        if (tooManyRemoteDeletes || tooManyLocalDeletes) {
          ...
          actions = actions.filter((a2) => {
            if (a2.type === "deleteRemote" && tooManyRemoteDeletes) {
              result.skippedDeletes++;
              return false;
            }
            if (a2.type === "deleteLocal" && tooManyLocalDeletes) {
              result.skippedDeletes++;
              return false;
            }
            return true;
          });
          const msg = `Mass deletion prevented (${[...skippedTypes].join(" & ")}). Would have deleted ${deleteRemoteCount} remote / ${deleteLocalCount} local out of ${trackedCount} tracked files. Run "Force upload all" or manually delete to resolve.`;
          result.errors.push(msg);
          new import_obsidian3.Notice(`OVH Sync: ${msg}`, 1e4);
        }
      }
      ...
      const localPaths = new Set(localFiles.map((f2) => f2.path));
      const remotePaths = new Set(remoteFiles.map((f2) => f2.path));
  ...
  async executeAction(action, metadata) {
    switch (action.type) {
      case "upload":
      case "conflict": {
        const doUpload = action.type === "upload" || action.type === "conflict" && action.resolution === "upload";
        if (doUpload) {
  ...
  async uploadFileWithMeta(path, metadata) {
    const file = this.vault.getAbstractFileByPath(path);
    if (!(file instanceof import_obsidian3.TFile)) return;
    const content = await this.vault.readBinary(file);
    const mtime = file.stat.mtime;
  ...
  async deleteLocalFile(path, metadata) {
    const file = this.vault.getAbstractFileByPath(path);
  ...
  async deleteRemoteAndTrack(path, metadata) {
    await this.s3.deleteObject(path);
    delete metadata.files[path];
  }
  async ensureDir(dir) {
...
var StatusBar = class {
  ...
  setResult(uploaded, downloaded, errors) {
    if (errors > 0) {
...
// src/main.ts
var OvhCloudSync = class extends import_obsidian4.Plugin {
  constructor() {
    super(...arguments);
  ...
  async onload() {
    ...
    this.debouncedProcessChanges = (0, import_obsidian4.debounce)(
      () => this.processChanges(),
  ...
  async loadSettings() {
    ...
    if (this.syncMetadata.lastFullSync > 0 && Object.keys(this.syncMetadata.files).length === 0) {
      this.syncMetadata = emptyMetadata();
    }
  ...
  async migrateLegacyCredentials() {
    ...
    if (migrated) {
      ...
      new import_obsidian4.Notice("OVH Cloud Sync: Credentials migrated to secure storage.");
    }
  }
  async saveSettings() {
    await this.saveData({
  ...
  getExcludePatterns() {
    return this.settings.excludePatterns.split("\n").map((p2) => p2.trim()).filter((p2) => p2.length > 0);
  }
  isConfigured() {
    const { accessKey, secretKey } = resolveCredentials(this.app, this.settings);
  ...
  onFileChange(file) {
    ...
    if (tracked && Math.abs(file.stat.mtime - tracked.mtime) <= 1e3 && file.stat.size === tracked.size) {
      return;
    }
  ...
  onFileDelete(file) {
    ...
    if (this.syncMetadata.files[file.path]) {
      this.syncEngine.deleteRemoteFile(file.path).catch((e2) => {
        console.error(`OVH Sync: failed to delete remote ${file.path}`, e2);
      });
    }
  }
  onFileRename(file, oldPath) {
    ...
    if (this.syncMetadata.files[oldPath]) {
      this.syncEngine.deleteRemoteFile(oldPath).catch((e2) => {
        console.error(`OVH Sync: failed to delete remote ${oldPath}`, e2);
      });
    }
  ...
  async processChanges() {
    ...
    for (const path of paths) {
      try {
        ...
        errors++;
        console.error(`OVH Sync: failed to upload ${path}`, e2);
      }
    }
  ...
  async runFullSync() {
    if (!this.isConfigured()) {
      new import_obsidian4.Notice("OVH Cloud Sync: Please configure your S3 credentials first.");
      return;
    }
    if (this.syncEngine.isSyncing) {
      new import_obsidian4.Notice("OVH Cloud Sync: Sync already in progress.");
      return;
    }
    this.statusBar.setSyncing();
    new import_obsidian4.Notice("OVH Cloud Sync: Starting sync...");
    ...
    if (result.errors.length > 0) {
      ...
      new import_obsidian4.Notice(`OVH Cloud Sync: ${parts.join(", ")}`);
    }
  ...
  async forceUploadAll() {
    ...
    this.statusBar.setSyncing();
    new import_obsidian4.Notice("OVH Cloud Sync: Uploading all files...");
    const files = this.app.vault.getFiles().filter(
      (f2) => !this.isExcluded(f2.path)
    );
    let uploaded = 0;
    let errors = 0;
    for (const file of files) {
      try {
        ...
        errors++;
        console.error(`OVH Sync: failed to upload ${file.path}`, e2);
      }
    }
    new import_obsidian4.Notice(`OVH Cloud Sync: Uploaded ${uploaded} files, ${errors} errors.`);
